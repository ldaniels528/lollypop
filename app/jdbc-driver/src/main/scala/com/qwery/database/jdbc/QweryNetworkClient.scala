...
import com.qwery.database.server.QweryServer
...
/**
 * Qwery Network Client
 */
trait QweryNetworkClient {
 ...
 * Qwery Network Client
 */
object QweryNetworkClient extends QweryNetworkClient {
  import scala.Console._

  private val choices = Seq(
    s"Start ${GREEN}dedicated Qwery Server$RESET",
    s"Start CLI with ${GREEN}embedded Qwery Server$RESET",
    s"Start CLI and connect to ${BLUE}remote Qwery Server$RESET",
  ...
  private def startDedicatedServer(port: Int = readInt("Server port: ")): QweryServer = {
  ...
  private def startCliWithEmbeddedServer(host: String = "0.0.0.0", port: Int = readInt("Server port: ")): Unit = {
    Console.println(s"Starting Local Database Server on port $port...")
    QweryServer(port) use { _ =>
      getConnection(jdbcDriver = classOf[QweryDriver].getName, jdbcURL = createURL(host, port)) use { conn =>
        interact(conn)
      }
    }
  }
  ...
  private def startCliWithRemoteServer(host: String = readString("Host: "),
    ...
    getConnection(jdbcDriver = classOf[QweryDriver].getName, jdbcURL = createURL(host, port)) use { conn => interact(conn) }
  }
  ...
  def startScript(): Unit = {
    ...
    getConnection(classOf[QweryDriver].getName, createURL(host, port)) use { implicit conn =>
  ...
  def createURL(host: String, port: Int): String = {
    s"jdbc:qwery://$host:$port/$DEFAULT_DATABASE.$DEFAULT_SCHEMA"
  }
