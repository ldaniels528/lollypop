...
import com.qwery.runtime.DatabaseManagementSystem.PatternSearchWithOptions
import com.qwery.runtime.DatabaseObjectConfig.implicits.RichDatabaseEntityConfig
import com.qwery.runtime.DatabaseObjectNS.{configExt, readConfig}
 ...
 * @param ctx the [[QweryUniverse]]
 */
class OS(ctx: QweryUniverse) {
  ...
  def compile(file: File): Instruction = new FileInputStream(file).use(QweryCompiler().compile)

  def compile(in: InputStream): Instruction = QweryCompiler().compile(in)
  ...
  def run(code: Instruction): Any = QweryVM.execute(Scope(), code)._3

  override def toString: String = "qwery.lang.OS"
...
object OS {
   ...
   * @return the size (in bytes) of an object or -1 if not a Qwery object
   */
  ...
  class StdInput(ctx: QweryUniverse) extends ConsoleIO {
  ...
  class StdOutput(ctx: QweryUniverse, writer1: PrintStream) extends ConsoleIO {
