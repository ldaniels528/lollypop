...
import com.qwery.language.models.{AllFields, Condition, Expression, ScopeModification}
import com.qwery.language.{dieIllegalType, dieNoSuchColumn}
import com.qwery.runtime.QweryVM.implicits.RichScalaAny
import com.qwery.runtime._
import com.qwery.runtime.datatypes.{IBLOB, TableType}
...
import com.qwery.runtime.instructions.conditions.{RuntimeCondition, WhereIn}
...
trait RowCollection extends RecordCollection[Row] with DataObject with SQLSupport
  ...
  def insertRows(columns: Seq[String], rowValues: Seq[Seq[Expression]])(implicit scope: Scope): IOCost = {
    (for {
      values <- rowValues.map(_.map(QweryVM.execute(scope, _)._3))
  ...
  def updateRow(rowID: ROWID, modification: ScopeModification, row: Row)(implicit scope: Scope): IOCost = {
    val (scope1, cost1, _) = QweryVM.execute(scope, modification)
