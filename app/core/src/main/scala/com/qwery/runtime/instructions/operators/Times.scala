...
case class Times(a: Expression, b: Expression) extends BinaryOperation {

  override def operator: String = "*"

}

object Times extends ExpressionChainParser {
  ...
  override def parseExpressionChain(stream: TokenStream, host: Expression)(implicit compiler: SQLCompiler): Option[Expression] = {
    stream match {
      case ts if ts nextIf "*" => for (b <- compiler.nextExpression(ts)) yield Times(host, b)
      case ts if ts nextIf "*=" => for (b <- compiler.nextExpression(ts)) yield Times(host, b).doAndSet
      case _ => None
    }
  }
