 ...
 * >> operator
 */
case class GreaterGreater(a: Expression, b: Expression) extends BinaryOperation {

  override def operator: String = ">>"

}

object GreaterGreater extends ExpressionChainParser {
  ...
  override def parseExpressionChain(stream: TokenStream, host: Expression)(implicit compiler: SQLCompiler): Option[Expression] = {
    stream match {
      case ts if ts nextIf ">>" => for (b <- compiler.nextExpression(ts)) yield GreaterGreater(host, b)
      case ts if ts nextIf ">>=" => for (b <- compiler.nextExpression(ts)) yield GreaterGreater(host, b).doAndSet
      case _ => None
    }
  }
