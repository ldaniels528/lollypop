...
case class TimesTimes(a: Expression, b: Expression) extends BinaryOperation {

  override def operator: String = "**"

}

object TimesTimes extends ExpressionChainParser {
  ...
  override def parseExpressionChain(stream: TokenStream, host: Expression)(implicit compiler: SQLCompiler): Option[Expression] = {
    stream match {
      case ts if ts nextIf "**" => for (b <- compiler.nextExpression(ts)) yield TimesTimes(host, b)
      case ts if ts nextIf "**=" => for (b <- compiler.nextExpression(ts)) yield TimesTimes(host, b).doAndSet
      case _ => None
    }
  }
