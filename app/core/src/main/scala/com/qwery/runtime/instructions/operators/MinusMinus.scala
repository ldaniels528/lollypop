...
case class MinusMinus(a: Expression, b: Expression) extends BinaryOperation {

  override def operator: String = "--"

}

object MinusMinus extends ExpressionChainParser {
  ...
  override def parseExpressionChain(stream: TokenStream, host: Expression)(implicit compiler: SQLCompiler): Option[Expression] = {
    stream match {
      case ts if ts nextIf "--" => for (b <- compiler.nextExpression(ts)) yield MinusMinus(host, b)
      case ts if ts nextIf "--=" => for (b <- compiler.nextExpression(ts)) yield MinusMinus(host, b).doAndSet
      case _ => None
    }
  }
