...
import com.qwery.language.dieUnsupportedEntity
...
import com.qwery.language.models.{AllFields, Condition, Expression, FieldRef, Function, Inequality, Literal, OrderColumn, Queryable}
import com.qwery.runtime.QweryVM.sort
import com.qwery.runtime.SQLRuntimeSupport.ColumnValidation
import com.qwery.runtime.datatypes.Inferences.{InstructionTyping, resolveType}
import com.qwery.runtime.datatypes.{DataType, Inferences, TableType}
import com.qwery.runtime.devices.RecordCollectionZoo.MapToRow
import com.qwery.runtime.devices.RowCollectionZoo.{createMemoryTable, createQueryResultTable}
...
trait SQLRuntimeSupport {
  ...
  private def selectAndAggregate(scope: Scope, select: Select): (IOCost, RowCollection) = {
    val (scope0, cost0, source) = select.from.map(QweryVM.search(scope, _)) || select.dieMissingSource()
  ...
  private def selectAndProduce(select: Select)(implicit scope: Scope): (IOCost, RowCollection) = {
    val results = select.fields.map {
      ...
      case expr =>
        val value = QweryVM.execute(scope, expr)._3
  ...
  private def selectAndTransform(select: Select)(implicit scope: Scope): (IOCost, RowCollection) = {
    val (_, cost0, source) = select.from.map(QweryVM.search(scope, _)) || select.dieMissingSource()
    ...
    val cost1 = source.iterateWhere(select.where, select.limit)(_.isActive) { (rowScope, incomingRow) =>
      val rawValues = selectedFields.map(QweryVM.execute(rowScope, _)._3)
  ...
  private def selectAndInnerJoin(scope0: Scope, select: Select): (IOCost, RowCollection) = {
    ...
    joinRows(primary, secondaries, select.limit) { case (scope2, row) =>
      ...
      val (_, c2, values) = QweryVM.transform(scope2, selectedFields)
  ...
  private def createJoinDevice(q: Queryable, joinColumnNames: String*)(implicit scope: Scope): (IOCost, RowCollection with CursorSupport) = {
    val (_, cost0, rc0) = QweryVM.search(scope, q)
