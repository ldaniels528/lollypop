...
import com.qwery.database.QueryRequest
import com.qwery.language.models._
import com.qwery.language.{ColumnTypeParser, Template, TokenStream, dieIllegalType}
...
import com.qwery.util.JSONSupport.{AnyToSprayJsConversion, JsValueConversion}
...
object ModelsJsonProtocol extends DefaultJsonProtocol with SprayJsonSupport {
  ...
  final implicit object ColumnTypeJsonFormat extends JsonFormat[ColumnType] {
    override def read(json: JsValue): ColumnType = {
      ...
      ColumnTypeParser.nextColumnType(TokenStream(sql))(QweryCompiler())
    }
  ...
  final implicit object expressionJsonFormat extends JsonFormat[Expression] {
    override def read(json: JsValue): Expression = {
      ...
      QweryCompiler().nextExpression(ts) || die(s"Could not compile: $sql")
    }
