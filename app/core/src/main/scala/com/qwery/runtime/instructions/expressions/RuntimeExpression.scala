...
import com.qwery.runtime.instructions.functions.{AnonymousFunction, ArgumentBlock}
...
trait RuntimeExpression extends Expression with RuntimeInstruction {
  ...
  def processInternalOps(host: Expression, args: List[Expression])(implicit scope: Scope): Any = {
    val instance = QweryVM.execute(scope, host)._3
    val values = QweryVM.evaluate(scope, args)
    val tuple = this
    instance match {
      // array(5) | ['A' to 'Z'](8, 19)
      case array: Array[_] =>
