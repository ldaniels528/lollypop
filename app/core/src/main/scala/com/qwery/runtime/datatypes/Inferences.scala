...
import com.qwery.language.{QweryUniverse, dieUnsupportedType}
import com.qwery.runtime.QweryNative
import com.qwery.runtime.instructions.expressions._
import com.qwery.runtime.instructions.functions.Iff
...
trait Inferences {
  ...
  def fromClass(`class`: Class[_]): DataType = `class` match {
    ...
    case c =>
      QweryUniverse().dataTypeParsers.collectFirst {
  ...
  def inferType(instruction: Instruction): DataType = {
    ...
    def recurse(opCode: Instruction): DataType = opCode match {
      case q: QweryNative => q.returnType
      // specific
