...
case class LessLessLess(a: Expression, b: Expression) extends BinaryOperation {

  override def operator: String = "<<<"

}

object LessLessLess extends ExpressionChainParser {
  ...
  override def parseExpressionChain(stream: TokenStream, host: Expression)(implicit compiler: SQLCompiler): Option[Expression] = {
    stream match {
      case ts if ts nextIf "<<<" => for (b <- compiler.nextExpression(ts)) yield LessLessLess(host, b)
      case ts if ts nextIf "<<<=" => for (b <- compiler.nextExpression(ts)) yield LessLessLess(host, b).doAndSet
      case _ => None
    }
  }
