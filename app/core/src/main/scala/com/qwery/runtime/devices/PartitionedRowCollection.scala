...
import com.qwery.runtime.devices.PartitionedRowCollection.PartitionedRange
import com.qwery.runtime.instructions.conditions.RuntimeCondition.RichConditionAtRuntime
import com.qwery.runtime.instructions.conditions.{EQ, Is}
import com.qwery.runtime.instructions.expressions.RuntimeExpression.RichExpression
import com.qwery.runtime.{DatabaseObjectNS, DatabaseObjectRef, QweryVM, ROWID, Scope, safeCast}
...
class PartitionedRowCollection[T](val ns: DatabaseObjectNS,
  ...
  override def iterateWhere(condition: Option[Condition] = None, limit: Option[Expression] = None)
    ...
    val searchKeys = QweryVM.evaluate(scope, searchKeyExpressions).map(safeCast[T])
