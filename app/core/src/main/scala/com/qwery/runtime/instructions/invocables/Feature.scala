...
import com.qwery.language.models.{CodeBlock, Condition, Expression, Instruction}
...
import com.qwery.runtime.instructions.conditions.{RuntimeCondition, Verification}
...
case class Feature(title: Expression, scenarios: Seq[Instruction]) extends RuntimeInvokable {
  ...
  override def invoke()(implicit scope: Scope): (Scope, IOCost, Any) = {
    ...
    val featureState = scenarios.foldLeft[FeatureState](fs0) {
      ...
      case (state, scenario: Scenario) =>
        val (scopeA, costA, resultA) = QweryVM.execute(resolveScope(scenario, state), scenario)
        if (resultA == true) completedScenario(scenario, state, scopeA)
        else failedScenario(scenario, state, scopeA)
      // any other instruction
      case (state, instruction) =>
        val (scopeA, costA, _) = QweryVM.execute(state.scope, instruction)
  ...
  private def resolveScope(scenario: Scenario, featureState: FeatureState)(implicit scope: Scope): Scope = {
    ...
    scenario.inherits match {
      case Some(titleExpr) =>
        QweryVM.execute(scope, titleExpr)._3 match {
