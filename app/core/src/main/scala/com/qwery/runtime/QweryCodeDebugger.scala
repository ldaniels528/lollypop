...
import com.qwery.language.models.{CodeBlock, Instruction, Queryable}
import com.qwery.runtime.QweryCodeDebugger.QApplication
import com.qwery.runtime.instructions.invocables.InlineCodeBlock
...
/**
 * Simple Code Debugger for Qwery
 */
trait QweryCodeDebugger {

  def stepThrough(file: String, console: () => String)(implicit compiler: QweryCompiler): Scope = {
    stepThrough(app = new QApplication(new File(file)), console)
  }

  def stepThrough(file: File, console: () => String)(implicit compiler: QweryCompiler): Scope = {
    stepThrough(app = new QApplication(file), console)
  }

  def stepThrough(app: QApplication, console: () => String)(implicit compiler: QweryCompiler): Scope = {
...
object QweryCodeDebugger {

  import scala.Console._

  def apply(): QweryCodeDebugger = {
    new QweryCodeDebugger {}
  }
  ...
  class QApplication(file: File, initialScope: Scope = Scope())(implicit compiler: SQLCompiler) {
    ...
    def executeCode(): Unit = {
      getInstruction foreach { op =>
        val (scope0, _, result0) = QweryVM.execute(scope, op)
        scope = scope0
        result = result0
        position += 1
        showCode()
