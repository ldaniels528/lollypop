...
/**
 * Abstract class for two-argument ([[Expression]]) [[FunctionCallParser function call parsers]]
 ...
 */
abstract class FunctionCallParserE2(name: String,
                                    description: String,
  ...
  def apply(expr1: Expression, expr2: Expression): InternalFunctionCall

  override def getFunctionCall(args: List[Expression]): Option[FunctionCall] = {
    (args.length == 2) ==> args.extract2 ~> { case (a, b) => apply(a, b) }
  }

  override def parseFunctionCall(ts: TokenStream)(implicit compiler: SQLCompiler): Option[FunctionCall] = {
    val params = SQLTemplateParams(ts, template)
    Option(apply(params.expressions("expr1"), params.expressions("expr2")))
  }
