...
import com.qwery.language.models.{Atom, Expression, Instruction, Queryable}
...
import com.qwery.runtime.instructions.expressions.Each.EnrichedBlockDevice
...
case class Each(variable: Atom,
  ...
  private def run()(implicit scope: Scope): RowCollection = {
    ...
    def processRow(row: Row): Option[Any] = {
      ...
      Option(QweryVM.execute(newScope, code)._3)
    }
    ...
    val rc = QweryVM.search(scope, queryable)._3
