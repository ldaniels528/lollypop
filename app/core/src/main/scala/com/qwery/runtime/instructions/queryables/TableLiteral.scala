...
import com.qwery.language.Token.TableToken
import com.qwery.language.models.Expression.implicits.LifestyleExpressionsAny
import com.qwery.language.models.{Expression, FieldRef, Literal}
import com.qwery.language.{ExpressionParser, HelpDoc, QueryableParser, SQLCompiler, TokenStream}
...
import com.qwery.util.Tabulator.tabulate
...
case class TableLiteral(columns: List[TableColumn], value: List[List[Expression]])
  ...
  override def search()(implicit scope: Scope): (Scope, IOCost, RowCollection) = {
    ...
    val cost = (for {
      values <- value.map(_.map(QweryVM.execute(scope, _)._3))
