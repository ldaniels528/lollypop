...
import com.qwery.language.{HelpDoc, QueryableParser, SQLCompiler, SQLTemplateParams, TokenStream}
import com.qwery.runtime.datatypes.{Inferences, TableType}
 ...
 * @param values the given collection of [[List row]]s
 */
  ...
  override def toTable(implicit scope: Scope): RowCollection = {
    ...
    for {
      ...
      mapping = Map((columns.map(_.name) zip row).map { case (name, expr) =>
        name -> QweryVM.execute(scope, expr)._3
      }: _*)
