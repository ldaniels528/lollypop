...
case class Minus(a: Expression, b: Expression) extends BinaryOperation {

  override def operator: String = "-"

}

object Minus extends ExpressionChainParser {
  ...
  override def parseExpressionChain(stream: TokenStream, host: Expression)(implicit compiler: SQLCompiler): Option[Expression] = {
    stream match {
      case ts if ts nextIf "-" => for (b <- compiler.nextExpression(ts)) yield Minus(host, b)
      case ts if ts nextIf "-=" => for (b <- compiler.nextExpression(ts)) yield Minus(host, b).doAndSet
      case _ => None
    }
  }
