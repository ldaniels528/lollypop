...
import com.qwery.runtime.errors._
...
trait InstructionErrors { self: Instruction =>
  ...
  def die(message: String): Nothing = throw QweryException(s"$message $toMessage".trim)

  def die(message: String, cause: Throwable): Nothing = throw QweryException(s"$message $toMessage".trim, cause)
