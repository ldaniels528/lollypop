...
import com.qwery.AppConstants.{__port__, qwerySessionID}
import com.qwery.database.QueryResponse.{QueryResultConversion, RESULT_ROWS}
...
import com.qwery.{AppConstants, die}
 ...
 * @param ctx  the root [[QweryUniverse compiler context]]
 */
class QweryServer(port: Int, ctx: QweryUniverse = QweryUniverse())(implicit system: ActorSystem) extends AutoCloseable {
  ...
  private def route: Route = {
    ...
    path("d" / Segment / Segment / Segment)(routesByDatabaseTable) ~
      ...
      cookie(qwerySessionID)(routesBySession)
  }
        ...
        val response = apiRoutes.collectFirst { case (url, mappings) if mappings.exists { case (method, _) => method == "ws" } =>
          mappings("ws") match {
            case af: AnonymousFunction =>
              val (scopeA, _, resultA) = QweryVM.execute(scope, af.call(List(tm.getStrictText.v)))
              scope = scopeA
              resultA
            case code: Instruction =>
              val (scopeA, _, resultA) = QweryVM.execute(scope, code)
              scope = scopeA
              resultA
            case other =>
              other
        ...
        def executeService(scopeA: Scope, method: String, input: Map[String, Any]): JsValue = {
          ...
          methods.get(method) flatMap {
            case instruction: Instruction =>
              ...
              val (scopeC, _, mixed) = QweryVM.execute(scopeB, instruction)
              Option(mixed) flatMap {
                case fx: AnonymousFunction =>
                  ...
                  Option(QweryVM.execute(scopeC, fx.call(args))._3)
                case op: Instruction => Option(QweryVM.execute(scopeC, op)._3)
  ...
  private def processWWW(file: File): Route = {
    if (file.getName.toLowerCase().endsWith(".md")) {
      ...
      val page = QweryPage.fromFile(file)
  ...
  private def routesByQuery: Route = {
    optionalCookie(qwerySessionID) { cookiePair =>
  ...
  private def runQuery(scope1: Scope, sql: String, limit: Option[Int]): (Scope, Route) = {
    ...
    val (scope2, cost1, result1) = QweryVM.execute(scope1, compiledCode)
    result1 match {
  ...
  private def toExpressions(jsObject: JsObject)(implicit scope: Scope, rc: RowCollection): Row = {
    Map(jsObject.fields.toSeq.map { case (k, js) =>
      k -> Option(QweryVM.execute(scope, js.toExpression)._3)
    }: _*).toRow
  }
...
/**
 * Qwery Server Companion
 */
object QweryServer {
  private val defaultActorPoolName = "qwery-server"

  def apply(port: Int): QweryServer = {
    implicit val system: ActorSystem = ActorSystem(name = defaultActorPoolName)
    new QweryServer(port)
  }

  def apply(port: Int, ctx: QweryUniverse): QweryServer = {
    implicit val system: ActorSystem = ActorSystem(name = defaultActorPoolName)
    new QweryServer(port, ctx)
  }

  def apply(port: Int, ctx: QweryUniverse, system: ActorSystem): QweryServer = {
    new QweryServer(port, ctx)(system)
  }
