...
/**
 * Abstract class for three-argument ([[Expression]]) [[FunctionCallParser function call parsers]]
 ...
 */
abstract class FunctionCallParserE3(name: String,
                                    description: String,
                                    examples: List[String],
                                    category: String,
                                    paradigm: String)
  extends FunctionCallParser(name, description, examples, template = s"$name ( %e:expr1 , %e:expr2 , %e:expr3 )", category, paradigm) {
  ...
  def apply(expr1: Expression, expr2: Expression, expr3: Expression): InternalFunctionCall

  override def getFunctionCall(args: List[Expression]): Option[FunctionCall] = {
    (args.length == 3) ==> args.extract3 ~> { case (a, b, c) => apply(a, b, c) }
  }

  override def parseFunctionCall(ts: TokenStream)(implicit compiler: SQLCompiler): Option[FunctionCall] = {
    val params = SQLTemplateParams(ts, template)
    Option(apply(params.expressions("expr1"), params.expressions("expr2"), params.expressions("expr3")))
  }
