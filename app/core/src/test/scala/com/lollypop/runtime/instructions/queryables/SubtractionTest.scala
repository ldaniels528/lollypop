...
class SubtractionTest extends AnyFunSpec with VerificationTools {
  private val logger = LoggerFactory.getLogger(getClass)
  implicit val compiler: LollypopCompiler = LollypopCompiler()

  describe(classOf[Subtraction].getSimpleName) {

    it("should support select .. subtract") {
    ...
    it("should decompile select .. subtract") {
      verify(
        s"""|select Symbol, Name, Sector, Industry, SummaryQuote
            |from Customers
            |where Industry == 'Oil/Gas Transmission'
    ...
    it("should perform a subtract between sources") {
      val (_, _, device) = LollypopVM.searchSQL(Scope(), sql =
        s"""|namespace 'samples.stocks'
            |drop if exists subtract_stocks
            |create table subtract_stocks (
            |   symbol: String(8),
            |   exchange: String(8),
            |   lastSale: Double,
            |   transactions Table (
            ...
            |   )[2]
            |)
            |insert into subtract_stocks (symbol, exchange, lastSale, transactions)
            ...
            |select symbol, exchange from subtract_stocks
            |  subtract
            |select symbol, exchange from subtract_stocks
      ...
      assert(device.toMapGraph == List(
        Map("symbol" -> "GMTQ", "exchange" -> "OTCBB"),
