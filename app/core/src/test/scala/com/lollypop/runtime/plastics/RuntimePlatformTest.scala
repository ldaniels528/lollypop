...
import com.lollypop.runtime.{LollypopCompiler, LollypopVM, Scope, safeCast}
...
class RuntimePlatformTest extends AnyFunSpec {
  ...
  describe("Any:toJson()") {
    it("should execute: [{Hello:'World'}].toJsonString()") {
      val (_, _, results) = LollypopVM.executeSQL(Scope(), """[{Hello:'World'}].toJsonString()""")
  ...
  describe("Any:toJsonPretty()") {
    it("should execute: [{Hello:'World'}].toJsonPretty()") {
      val (_, _, results) = LollypopVM.executeSQL(Scope(), """[{Hello:'World'}].toJsonPretty()""")
      assert(results ==
        """|[{
           |  "Hello": "World"
           |}]""".stripMargin)
  ...
  describe("Any:toPrettyString()") {
    it("should execute the 'toPrettyString()' function") {
      val (_, _, results) = LollypopVM.executeSQL(Scope(), "'Hello'.getBytes().toPrettyString()")
  ...
  describe("Array:contains()") {
    it("should execute: [1, 3, 5, 7, 9].contains(5)") {
      val (_, _, result) = LollypopVM.executeSQL(Scope(), """[1, 3, 5, 7, 9].contains(5)""")
      assert(result == true)
    }
  ...
  describe("Array:distinctValues()") {
    it("should execute: [1, 3, 5, 7, 9, 7, 3, 2, 1].distinctValues()") {
      val (_, _, results) = LollypopVM.executeSQL(Scope(), """[1, 3, 5, 7, 9, 7, 3, 2, 1].distinctValues()""")
      assert(toList(results) == List(1, 3, 5, 7, 9, 2))
    }

    it("should execute: [1, 3, 5, 7, 9, 7, 3, 2, 1, null].distinctValues()") {
      val (_, _, results) = LollypopVM.executeSQL(Scope(), """[1, 3, 5, 7, 9, 7, 3, 2, 1, null].distinctValues()""")
      assert(toList(results) == List(1, 3, 5, 7, 9, 2, null))
    }

    it("should execute: [1, 3, 5, 7].foldLeft(0, (a: Int, b: Int) => a + b)") {
      val (_, _, results) = LollypopVM.executeSQL(Scope(), """[1, 3, 5, 7].foldLeft(0, (a: Int, b: Int) => a + b)""")
      assert(results == 16)
    }
  }
  ...
  describe("Array:join()") {
    it("""should execute: ['A', 'p', 'p', 'l', 'e', '!'].join("")""") {
      val (_, _, result) = LollypopVM.executeSQL(Scope(), """['A', 'p', 'p', 'l', 'e', '!'].join("")""")
      assert(result == "Apple!")
    }
    ...
    it("should execute: [1, 3, 5, 7, 9].join(', ')") {
      val (_, _, result) = LollypopVM.executeSQL(Scope(), """[1, 3, 5, 7, 9].join(', ')""")
  ...
  describe("Array:length()") {
    it("should execute: [1 to 5].length()") {
      val (_, _, result) = LollypopVM.executeSQL(Scope(), """[1 to 5].length()""")
      assert(result == 5)
    }
  }
  ...
  describe("Array:map()") {
    it("should execute: [1 to 5].map((n: int) => n * n)") {
      val (_, _, results) = LollypopVM.executeSQL(Scope(), """[1 to 5].map((n: int) => n * n)""")
  ...
  describe("Array:maxValue()") {
    it("should execute: [1, 3, 5, 7, 9].maxValue()") {
      val (_, _, result) = LollypopVM.executeSQL(Scope(), "[1, 3, 5, 7, 9].maxValue()")
      assert(result == 9)
    }

    it("should execute: [1, 13, 3, 5, null, 7, 9].maxValue()") {
      val (_, _, result) = LollypopVM.executeSQL(Scope(), "[1, 13, 3, 5, null, 7, 9].maxValue()")
      assert(result == 13)
    }
  }
  ...
  describe("Array:minValue()") {
    it("should execute: [1, 3, 5, 7, 9].minValue()") {
      val (_, _, result) = LollypopVM.executeSQL(Scope(), "[1, 3, 5, 7, 9].minValue()")
      assert(result == 1)
    }

    it("should execute: [1, 3, 5, 0, null, 7, 9].minValue()") {
      val (_, _, result) = LollypopVM.executeSQL(Scope(), "[1, 3, 5, 0, null, 7, 9].minValue()")
      assert(result == 0)
    }
  }
  ...
  describe("Array:push()") {
    it("should execute: ['A' to 'D'].push('Z')") {
      val (_, _, results) = LollypopVM.executeSQL(Scope(), """['A' to 'D'].push('Z')""")
      assert(toList(results) == List('A', 'B', 'C', 'D', 'Z'))
    }

    it("should execute: ['A' to 'C'].push(['X', 'Y', 'Z'])") {
      val (_, _, results) = LollypopVM.executeSQL(Scope(), """['A' to 'C'].push(['X', 'Y', 'Z'])""")
  ...
  describe("Array:reverse()") {
    ...
    it("""should execute: ['A' to 'Z'].reverse()""") {
      val (_, _, results) = LollypopVM.executeSQL(Scope(), """['A' to 'Z'].reverse()""")
  ...
  describe("Array:slice()") {
    it("should execute: ['A' to 'Z'].slice(1, 5)") {
      val (_, _, results) = LollypopVM.executeSQL(Scope(), """['A' to 'Z'].slice(1, 5)""")
  ...
  describe("Array:sortValues()") {
    it("should execute: [3, 9, 5, 2, 7, 1].sortValues()") {
      val (_, _, results) = LollypopVM.executeSQL(Scope(), """[3, 9, 5, 2, 7, 1].sortValues()""")
  ...
  describe("Bytes:fromHex(), toHex()") {
    it("""should execute: "deadbeef".fromHex().toHex()""") {
      val (_, _, results) = LollypopVM.executeSQL(Scope(), """"deadbeef".fromHex().toHex()""")
  ...
  describe("Bytes:gzip(), gunzip()") {
    it("""should execute: "Hello".getBytes().gzip()""") {
      val (_, _, results) = LollypopVM.executeSQL(Scope(), """String("Hello".getBytes().gzip().gunzip())""")
      assert(results == "Hello")
    }
  }
  ...
  describe("Bytes:md5(), toHex()") {
    it("""should execute: "Hello".getBytes().md5()""") {
      val (_, _, results) = LollypopVM.executeSQL(Scope(), """String("Hello".getBytes().md5().toHex())""")
  ...
  describe("Bytes:serialize(), deserialize()") {
    it("""should execute: "What did the fox say?".serialize().deserialize()""") {
      val (_, _, results) = LollypopVM.executeSQL(Scope(), """"What did the fox say?".serialize().deserialize()""")
  ...
  describe("Bytes:snappy(), unSnappy()") {
    it("""should execute: "Hello".getBytes().snappy()""") {
      val (_, _, results) = LollypopVM.executeSQL(Scope(), """String("Hello".getBytes().snappy().unSnappy())""")
      assert(results == "Hello")
    }
  }
  ...
  describe("Date:dayOfMonth()") {
    it("should execute: DateTime('2021-09-02T11:22:33.000Z').dayOfMonth()") {
      val (_, _, results) = LollypopVM.executeSQL(Scope(), "DateTime('2021-09-02T11:22:33.000Z').dayOfMonth()")
      assert(results == 2)
    }
  }
  ...
  describe("Date:dayOfWeek()") {
    it("should execute: DateTime('2021-09-02T11:22:33.000Z').dayOfWeek()") {
      val (_, _, results) = LollypopVM.executeSQL(Scope(), "DateTime('2021-09-02T11:22:33.000Z').dayOfWeek()")
      assert(results == 5)
    }
  }
  ...
  describe("Date:format()") {
    it("should execute: DateTime('2021-09-02T11:22:33.000Z').format('yyyy-MM-dd')") {
      val (_, _, results) = LollypopVM.executeSQL(Scope(), "DateTime('2021-09-02T11:22:33.000Z').format('yyyy-MM-dd')")
  ...
  describe("Date:hour()") {
    it("should execute: DateTime('2021-09-02T11:22:33.000Z').hour()") {
      val (_, _, result) = LollypopVM.executeSQL(Scope(), "DateTime('2021-09-02T11:22:33.000Z').hour()")
      assert(result == 11)
    }
  }
  ...
  describe("Date:millisecond()") {
    it("should execute: DateTime('2021-09-02T11:22:33.000Z').millisecond()") {
      val (_, _, result) = LollypopVM.executeSQL(Scope(), "DateTime('2021-09-02T11:22:33.375Z').millisecond()")
      assert(result == 375)
    }
  }
  ...
  describe("Date:minute()") {
    it("should execute: DateTime('2021-09-02T11:22:33.000Z').minute()") {
      val (_, _, result) = LollypopVM.executeSQL(Scope(), "DateTime('2021-09-02T11:22:33.000Z').minute()")
      assert(result == 22)
    }
  }
  ...
  describe("Date:month()") {
    it("should execute: DateTime('2021-09-02T11:22:33.000Z').month()") {
      val (_, _, result) = LollypopVM.executeSQL(Scope(), "DateTime('2021-09-02T11:22:33.000Z').month()")
      assert(result == 9)
    }
  }
  ...
  describe("Date:second()") {
    it("should execute: DateTime('2021-09-02T11:22:33.000Z').second()") {
      val (_, _, result) = LollypopVM.executeSQL(Scope(), "DateTime('2021-09-02T11:22:33.000Z').second()")
      assert(result == 33)
    }
  }
  ...
  describe("Date:split()") {
    it("should execute: DateTime('2021-09-02T11:22:33.000Z').split()") {
      val (_, _, result) = LollypopVM.executeSQL(Scope(), "DateTime('2021-09-02T11:22:33.357Z').split()")
  ...
  describe("Date:year()") {
    it("should execute: DateTime('2021-09-02T11:22:33.000Z').year()") {
      val (_, _, result) = LollypopVM.executeSQL(Scope(), "DateTime('2021-09-02T11:22:33.000Z').year()")
      assert(result == 2021)
    }
  }
  ...
  describe("Numeric:evaluate()") {
    it("""should execute: 23.asBinaryString()""") {
      val (_, _, result) = LollypopVM.executeSQL(Scope(), "23.asBinaryString()")
      assert(result == "10111")
    }
  }
  ...
  describe("String:fromJson()") {
    it("""should execute: '{ "name": "height", "value": 76 }'.fromJson()""") {
      val (_, _, result) = LollypopVM.executeSQL(Scope(), """'{ "name": "height", "value": 76 }'.fromJson()""")
      assert(result == Map("name" -> "height", "value" -> 76))
    }

    it("""should execute: ('{ "symbol": "ABC", "exchange": "NYSE" }'.fromJson()).symbol""") {
      val (_, _, result) = LollypopVM.executeSQL(Scope(), """('{ "symbol": "ABC", "exchange": "NYSE" }'.fromJson()).symbol""")
      assert(result == "ABC")
    }
  }
  ...
  describe("String:reverse()") {
    it("""should execute: "Hello World".reverse()""") {
      val (_, _, result) = LollypopVM.executeSQL(Scope(), """"Hello World".reverse()""")
  ...
  private def decompile(sql: String): Assertion = {
    val model = LollypopCompiler().compile(sql)
