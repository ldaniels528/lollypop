...
import com.qwery.util.JSONSupportTest.{Ticker, TickerPojo}
  ...
  describe(JSONSupport.getClass.getSimpleName.replaceAll("[$]", "")) {
    ...
    it("should convert a List (objects) to a JSON array") {
      verify(
        ...
        expected = JsArray(
          JsObject(Map("symbol" -> JsString("ABC"), "exchange" -> JsString("NYSE"), "lastSale" -> JsNumber(13.87), "_class" -> JsString("com.qwery.util.JSONSupportTest$Ticker"))),
          JsObject(Map("symbol" -> JsString("MEAT.OB"), "exchange" -> JsString("OTCBB"), "lastSale" -> JsNumber(0.0001), "_class" -> JsString("com.qwery.util.JSONSupportTest$Ticker"))),
          JsObject(Map("symbol" -> JsString("TREE"), "exchange" -> JsString("NASDAQ"), "lastSale" -> JsNumber(56.79), "_class" -> JsString("com.qwery.util.JSONSupportTest$Ticker")))
        ))
    }
    ...
    it("should convert a case class to a JSON object") {
      verify(
        ...
        expected = JsObject(Map(
          ...
          "_class" -> JsString("com.qwery.util.JSONSupportTest$Ticker")
        )))
    }
    ...
    it("should convert a POJO class to a JSON object") {
      verify(
        ...
        expected = JsObject(Map(
          ...
          "_class" -> JsString("com.qwery.util.JSONSupportTest$TickerPojo")
        )))
    }
