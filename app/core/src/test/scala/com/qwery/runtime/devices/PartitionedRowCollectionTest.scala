...
import com.qwery.runtime.datatypes.{DateTimeType, Float64Type, StringType}
...
class PartitionedRowCollectionTest extends AnyFunSpec {
  ...
  describe(classOf[PartitionedRowCollection[_]].getSimpleName) {
    ...
    it("should perform CRUD operations on durable partitioned tables") {
      ...
      val (scope1, cost1, result1) = QweryVM.searchSQL(scope0.reset(),
        """|select * from Stocks
      ...
      val (_, cost2, result2) = QweryVM.searchSQL(scope1.reset(),
        """|select * from Stocks
