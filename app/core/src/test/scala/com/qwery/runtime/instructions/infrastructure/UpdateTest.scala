...
import com.qwery.runtime.instructions.invocables.{ScopeModificationBlock, SetAnyVariable}
...
class UpdateTest extends AnyFunSpec with VerificationTools {
  ...
  describe(classOf[Update].getSimpleName) {
    ...
    it("should support compilation with +=") {
      ...
      val actual = QweryCompiler().compile(
        """|update Stocks
           |lastSale += 0.0001
    ...
    it("should support decompilation") {
      val model = QweryCompiler().nextOpCodeOrDie(TokenStream(
        """|update Stocks
           |set lastSale = 1.06
           |where symbol is 'ABC'
           |limit 1
