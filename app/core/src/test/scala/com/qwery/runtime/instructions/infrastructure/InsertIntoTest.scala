...
import com.qwery.runtime.instructions.queryables.{RowsOfValues, Select, TableLiteral}
...
class InsertIntoTest extends AnyFunSpec with VerificationTools {
  ...
  describe(classOf[InsertInto].getSimpleName) {
    ...
    it(s"should insert JSON data into a nested table in memory") {
      ...
      // verify the count
      val (_, _, result1) = QweryVM.searchSQL(scope0, sql = "select total: count(*) from @@stocks")
    ...
    it("should insert objects into a nested table in memory") {
      ...
      // verify the count
      val (_, _, result1) = QweryVM.searchSQL(scope0, s"select total: count(*) from @@stocks")
      assert(result1.toMapGraph == List(Map("total" -> 5)))
    }
