  ...
  describe(classOf[CreateType].getSimpleName) {
    ...
    it("should support custom types (CurrencyType)") {
      val (_, _, results) = QweryVM.searchSQL(Scope(),
        """|create type Currency as UDT('com.github.ldaniels528.qwery.CurrencyType')
    ...
    it(s"should support create table with custom Type") {
      ...
      val (scope2, _, device2) = QweryVM.searchSQL(scope1, sql =
      ...
      val (_, _, device3) = QweryVM.searchSQL(scope2, sql =
