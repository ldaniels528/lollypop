...
import com.qwery.runtime.devices.errors.FormatNotSpecified
...
class ExternalFilesTableRowCollectionTest extends AnyFunSpec with VerificationTools {
  ...
  describe(classOf[ExternalFilesTableRowCollection].getSimpleName) {
    ...
    it("should support indexed searches") {
      ...
      // retrieve a record
      val ((_, cost3, device3), ts3) = time(QweryVM.searchSQL(Scope(),
      ...
      val ((_, cost5, device5), ts5) = time(QweryVM.searchSQL(Scope(),
    ...
    it("should query rows using SQL") {
      val (_, _, rs) = QweryVM.searchSQL(Scope(),
