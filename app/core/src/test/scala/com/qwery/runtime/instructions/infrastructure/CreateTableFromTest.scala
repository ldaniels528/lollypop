...
import com.qwery.runtime.instructions.queryables.{RowsOfValues, Select}
import com.qwery.runtime.{DatabaseObjectRef, QweryCompiler, QweryVM, Scope}
import com.qwery.util.OptionHelper.implicits.risky._
import org.scalatest.funspec.AnyFunSpec

class CreateTableFromTest extends AnyFunSpec with VerificationTools {
  ...
  describe(classOf[CreateTableFrom].getSimpleName) {
    ...
    it("should compile create table containing inline values") {
      val results = compiler.compile(
       s"""|create table if not exists $specialSecuritiesRef (symbol: String, lastSale: Double)
           ...
           |""".stripMargin)
      assert(results == CreateTableFrom(specialSecuritiesRef,
