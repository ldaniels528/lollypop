...
class BLOBTest extends AnyFunSpec with VerificationTools {
  private val baseDir = new File("app") / "jdbc-driver" / "src" / "test" / "scala" / "com" / "qwery" / "database" / "jdbc"
  ...
  describe(classOf[BLOB].getSimpleName) {
    ...
    it("should create a BLOB from a RowCollection") {
      val (_, _, stocksBlob) = QweryVM.searchSQL(Scope(),
        """|val stocks = 
           | |---------------------------------------------------------|
    ...
    it("should search a BLOB for a pattern") {
      val blob = IBLOB(BLOB.fromFile(testFile))
      val pattern = IBLOB(BLOB.fromString("Class.forName(QweryDriver.getClass.getName)"))
    ...
    it("should search for and replace a slice of the BLOB") {
      val blob = IBLOB(BLOB.fromFile(testFile))
      val pattern = "Class.forName(QweryDriver.getClass.getName)".getBytes()
      val replacement = "Class.forName(DriverQwery.getClass.getName)".getBytes()
      ...
      // verify the slice
      assert(blob.getBinaryStream(231, replacement.length).mkString() == new String(replacement))
      blob.free()
    }
