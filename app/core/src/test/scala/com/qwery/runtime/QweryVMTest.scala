...
import com.qwery.language.dieNoResultSet
 ...
 */
class QweryVMTest extends AnyFunSpec {
  private val logger = LoggerFactory.getLogger(getClass)
  implicit val compiler: QweryCompiler = QweryCompiler()

  describe(classOf[QweryVM.type].getSimpleName) {
    ...
    it(s"should support string interpolation") {
      val (scope, _, _) = QweryVM.executeSQL(Scope(), sql =
    ...
    it("should support infix accessor: ({ total: 100 }).total") {
      val (_, _, value) = QweryVM.executeSQL(Scope(), sql = "({ total: 100 }).total")
      assert(value == 100)
    }

    it("""should support virtual infix accessor: ({ symbol: "ABC", exchange: "NYSE" }).symbol""") {
      val (_, _, value) = QweryVM.executeSQL(Scope(), sql = """({ symbol: "ABC", exchange: "NYSE" }).symbol""")
      assert(value == "ABC")
    }

    it("should support dictionary infix accessor: { estimate: ({ total: 1619 }}.estimate).total") {
      val (_, _, device) = QweryVM.searchSQL(Scope(), sql = "select n: ({ estimate: { total: 1619 }}.estimate).total")
    ...
    it("should be able to produce records containing Random.nextInt(om) data") {
      val (_, _, device_?) = QweryVM.executeSQL(Scope(),
    ...
    it("should use .swap() to swap the contents of two rows in a table") {
      val (_, _, returned) = QweryVM.executeSQL(Scope(),
        s"""|declare table Travelers(id: UUID, lastName: String(12), firstName: String(12), destAirportCode: String(3))
            | containing values
