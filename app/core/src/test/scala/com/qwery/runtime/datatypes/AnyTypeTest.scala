...
class AnyTypeTest extends DataTypeFunSpec with VerificationTools {
  ...
  describe(classOf[AnyType].getSimpleName) {
    ...
    it("should detect InstanceType column types") {
      verifyType(value = FakeNews(message = "Hello World"), expectedType = AnyType("com.qwery.runtime.datatypes.DataTypeFunSpec$FakeNews"))
    }

    it("should encode/decode InstanceType values") {
      verifyCodec(AnyType("com.qwery.runtime.datatypes.DataTypeTest$FakeNews"), value = FakeNews("Hello World"))
    }
