...
import com.qwery.runtime.datatypes.{Float64Type, Int32Type, Int64Type, StringType}
import com.qwery.runtime.devices.RecordCollectionZoo.MapToRow
import com.qwery.runtime.devices._
import com.qwery.runtime.instructions.queryables.{From, This}
...
class ScopeTest extends AnyFunSpec {
  ...
  describe(classOf[Scope].getSimpleName) {
    ...
    it("should override the database from a parent scope") {
      val (parentScope, _, _) = QweryVM.executeSQL(Scope(), "namespace 'slovett.work'")
      val (childScope, _, _) = QweryVM.executeSQL(Scope(parentScope = parentScope), "namespace 'ldaniels.work'")
      ...
      val scope = scope0
        .withCurrentRow(Some(Row(id = 1001, metadata = RowMetadata(), columns = columns, fields = columns.map(c =>
          Field(name = c.name, metadata = FieldMetadata(), value =
            c.defaultValue.flatMap(v => Option(QweryVM.execute(scope0, v)._3)).map(c.`type`.convert))
        ))))
      assert(scope.resolve("__id") contains 1001)
    ...
    it("should distinguish between variables and fields of the current row") {
      ...
      val scope = scope0
        ...
        .withCurrentRow(Some(Row(id = 1001, metadata = RowMetadata(), columns = columns, fields = columns.map(c =>
          Field(name = c.name, metadata = FieldMetadata(), value = c.defaultValue.flatMap(v => Option(QweryVM.execute(scope0, v)._3)).map(c.`type`.convert))
        ))))
      assert(QweryVM.executeSQL(scope, "symbol")._3 == "ABC")
      assert(QweryVM.executeSQL(scope, "@symbol")._3 == "BROK")
    }
    ...
    it("should provide the state for expressions") {
      ...
      val (_, _, result) = QweryVM.execute(scope, expr)
      assert(result == 14)
    }
    ...
    it("should query the `this` variable") {
      ...
      val (_, _, device) = QweryVM.search(scope, From(This()))
    ...
    it("should read the database from a parent scope") {
      val (parentScope, _, _) = QweryVM.executeSQL(Scope(), "namespace 'ldaniels.work'")
      val childScope = Scope(parentScope)
      assert(childScope.getDatabase contains "ldaniels")
      assert(childScope.getSchema contains "work")
    }
    ...
    it("should update the name upon realizing an object reference from a parent scope") {
      val (parentScope, _, _) = QweryVM.executeSQL(Scope(), "namespace `securities.stocks`")
      val childScope = Scope(parentScope)
      assert(DatabaseObjectRef("nasdaq").realize(childScope) == DatabaseObjectNS(databaseName = "qwery", schemaName = "public", name = "nasdaq"))
    }
    ...
    it("should execute methods of Scala-native implicit classes") {
      val scope = Scope().importImplicitClass(implicitClass = Class.forName("com.qwery.util.StringRenderHelper$StringRenderer"))
    ...
    it("should provide the Qwery version string") {
