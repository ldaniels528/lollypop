...
class HashTagTest extends AnyFunSpec {
  ...
  describe(classOf[HashTag].getSimpleName) {
    ...
    it("should compile: stocks#symbol") {
      assert(QweryCompiler().compile("stocks#symbol") == HashTag("stocks".f, "symbol".f))
    }

    it("should compile: stocks#[symbol, exchange]") {
      assert(QweryCompiler().compile("stocks#[symbol, exchange]") == HashTag("stocks".f, ArrayLiteral("symbol".f, "exchange".f)))
    }

    it("should compile/decompile: stocks#symbol") {
      assert(QweryCompiler().compile("stocks#symbol").toSQL == "stocks#symbol")
    }

    it("should compile/decompile: stocks#[symbol, exchange]") {
      assert(QweryCompiler().compile("stocks#[symbol, exchange]").toSQL == "stocks#[symbol, exchange]")
    }
