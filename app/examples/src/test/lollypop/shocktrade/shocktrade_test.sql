...
feature 'ShockTrade services' {
    ...
    // log all calls to the `www` instruction
    whenever '^www (delete|get|post|put) (.*)' {
   ...
   //////////////////////////////////////////////////////////////////////////////////////
    ...
    scenario 'Create a new contest' {
        val responseA = www post 'http://{{host}}:{{port}}/api/shocktrade/contests' <~ { name: "Winter is coming" }
    ...
    scenario 'Retrieve the contest' extends 'Create a new contest' {
        val responseB = www get 'http://{{host}}:{{port}}/api/shocktrade/contests?id={{contest_id}}'
    ...
    scenario 'Update an existing contest' {
        ...
        val responseC = www post 'http://{{host}}:{{port}}/api/shocktrade/contests' <~ { name: "Winter is here" }
        ...
        val responseD = www put 'http://{{host}}:{{port}}/api/shocktrade/contests?newName={{url_encode(newName)}}&id={{contest_id}}'
        verify responseD.statusCode is 200 and responseD.body is 1
            ^^^ "Updated contest: {{contest_id}}"
    }

    scenario 'Search for contests' {
        val responseE = www post 'http://{{host}}:{{port}}/api/shocktrade/contests/by/name' <~ { searchText: "Winter" }
   ...
   //////////////////////////////////////////////////////////////////////////////////////
    ...
    scenario 'Create a new member' {
        val responseF = www post 'http://{{host}}:{{port}}/api/shocktrade/members' <~ { name: "fugitive528" }
    ...
    scenario 'Retrieve the member' extends 'Create a new member' {
        val responseG = www get 'http://{{host}}:{{port}}/api/shocktrade/members?id={{memberID}}'
   ...
   //////////////////////////////////////////////////////////////////////////////////////
    ...
    scenario 'Join a member to a contest as a new participant' extends ['Create a new member', 'Create a new contest'] {
        val responseH = www post 'http://{{host}}:{{port}}/api/shocktrade/participants'
    ...
    scenario 'Retrieve the participant' extends 'Join a member to a contest as a new participant' {
        val responseI = www get 'http://{{host}}:{{port}}/api/shocktrade/participants?id={{participant_id}}'
   ...
   //////////////////////////////////////////////////////////////////////////////////////
    ...
    scenario 'Create a new order' extends 'Join a member to a contest as a new participant' {
        val responseJ = www post 'http://{{host}}:{{port}}/api/shocktrade/orders'
   ...
   scenario 'Retrieve the order' extends 'Create a new order' {
       val responseK = www get 'http://{{host}}:{{port}}/api/shocktrade/orders?id={{order_id}}'
   ...
   //////////////////////////////////////////////////////////////////////////////////////
    ...
    scenario 'Create a new position' extends 'Create a new order' {
        val responseL = www post 'http://{{host}}:{{port}}/api/shocktrade/positions'
                            <~ { contest_id: $contest_id, participant_id: $participant_id, order_id: $order_id,
   ...
   scenario 'Retrieve the position' extends 'Create a new position' {
       val responseM = www get 'http://{{host}}:{{port}}/api/shocktrade/positions?id={{position_id}}'
    ...
    scenario 'Retrieve the positions by participant' extends 'Create a new position' {
        val responseN = www get 'http://{{host}}:{{port}}/api/shocktrade/positions/by/participant?id={{participant_id}}'
